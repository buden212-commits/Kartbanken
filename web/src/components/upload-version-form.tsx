...
export function UploadVersionForm({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (activeCheckouts.length > 0 && !isAdmin) {
      setError(
        `Det finns ${activeCheckouts.length} aktiv(a) checkout(s). Vänta tills de är integrerade eller avbrutna.`,
      );
      return;
    }
    ...
    <>
      ...
      {pendingUpload?.kind === "checkout" && activeCheckouts.length > 0 && (
        <div className="mt-4 rounded-xl border border-amber-300 bg-amber-50 p-4">
          <HelpSectionHeading section="checkout">Aktiva checkouts</HelpSectionHeading>
          <p className="mt-2 text-sm text-amber-900">
            Det finns {activeCheckouts.length} aktiv(a) checkout(s). Full uppladdning kan ogiltigförglora
            pågående arbete och ändra head-versionen.
          </p>
